...
customRoutes = [
    ...
    //Rols Rout - GET/DELETE
    {
        path: '/rols',
        templateUrl: $base_url + 'views/path/rols.html',
        controller: 'RolsController',
        requiredRoles: ['admin'],
        requiresAuth: true,
    },
    //Rols Rout - CREATE
    {
        path: '/addrols',
        templateUrl: $base_url + 'views/path/rols_add.html',
        controller: 'RolsAddController',
        requiresAuth: true,
    },
    //Rols Rout - EDIT
    {
...
newModule.controller("EventsController", function ($scope, $rootScope, $location, $http, $timeout, UserService) {
    ...
    $scope.activeEvent = function (itemId) {
        addSpin("h1");
        $('.eventActive').prop("disabled", true);
        $http({
            method: 'POST',
            url: 'api/active/event/' + itemId,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Authorization': 'Basic ' + btoa(`${token}:`)
            },
            ...
            .then(function (response) {
                ...
                $scope.eventsArray.forEach(function callback(item, k) {
                    if (item._id.$oid == itemId) {
                        $scope.eventsArray[k].active = true;
                    } else {
                        $scope.eventsArray[k].active = false;
                    }
                });
                // console.log("eventActive:", response);
                $event = response.data.answer;
            })
            .catch(function (response) {
                delSpin(false);
                showAlert("danger", response.data.error);
            ...
            .finally(function () {
                $timeout(function () {
                    $('.eventActive').prop("disabled", false);
                    delSpin(true);
                });
            });
...
newModule.controller("AddEventsController", function ($scope, $rootScope, $location, $http, $timeout, UserService) {
    ...
    $scope.removeCampo = function (index) {
        $scope.evento.fields.splice(index, 1);
    };

    $scope.addOption = function (campoIndex) {
        console.log("Index", campoIndex)
        var campo = $scope.evento.fields[campoIndex];
    ...
    $scope.submitEvent = function () {
        ...
        // $formData.push($scope.evento.campos)
        $scope.evento.fields.forEach(function (campo, i) {
        ...
        // let $formData = new FormData($scope.evento);
...
newModule.controller("RolsController", function ($scope, $rootScope, $location, $http, $timeout, UserService) {
    // console.log("ROOT FILE Controller");
    $rootScope.title = `${title} - Rols`;
    $scope.h1 = "Rols";
    $rootScope.sidebar = $sidebar;
    $rootScope.user = UserService.getUser();
    ...
    $scope.deleterols = function (item) {
        $scope.toDelete = item._id.$oid;
        $scope.toDeleteName = item.name;
        $timeout(function () {
            $("#confirmDelete").focus();
...
newModule.controller("RolsAddController", function ($scope, $rootScope, $location, $http, $timeout, UserService) {
    $rootScope.title = `${title} - Agregar Rol`;
    $rootScope.sidebar = $sidebar;
    $scope.h1 = "Agregar Rol ";
    $scope.token = token;
    $scope.rols = $rols;
    $rootScope.user = UserService.getUser();
    ...
    $scope.genAlias = function () {
        if ($scope.rol && $scope.rol.name) {
            // Obtener el nombre del evento desde el modelo
            var nombreRol = $scope.rol.name;

            // Reemplazar espacios con guiones y convertir a minúsculas
            var alias = nombreRol.replace(/\s+/g, '-').toLowerCase();

            // Asignar el resultado al modelo del Slug
            $scope.rol.alias = alias;
        } else {
            $scope.rol.alias = '';
        }
    };
...
newModule.controller("RolsEditController", function ($scope, $rootScope, $location, $http, $timeout, $routeParams, UserService) {
    $rootScope.title = `${title} - Editar Evento`;
    $rootScope.sidebar = $sidebar;
    $scope.rols = $rols;
    $rootScope.user = UserService.getUser();
    let rolId = $routeParams.rolId;
    $scope.id = rolId;
    $scope.token = token;
    ...
    $http({
        ...
        .then(function (response) {
            // console.log(response.data);
            $scope.rol = response.data.answer;
            // $scope.h1 += `(${$scope.evento.name})`
        })
        .catch(function (response) {
            delSpin(false);
